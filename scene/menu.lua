...
scene.load = function(returned)
  ...
  scene.title:send("[dropshadow=10][rainbow]Trade Map[/rainbow][/dropshadow]")
  
  scene.subtext = sysl.new("center", syslSubText)
  scene.subtext:send("[dropshadow=10]An editor for trades[/dropshadow]")
...
local _directorydroppedError = function(path, errorMessage)
  print("Failed to open project at", path, ", because: ", errorMessage)
  showError(errorMessage)
end

scene.directorydropped = function(path)
  print("Attempting to open project at", path)
  ...
  print("Project data load, start loading project")

  local start = love.timer.getTime()
  local project, errorMessage = project.loadProject(path)

  if not project then
    _directorydroppedError(errorMessage)
    return
  end

  scene.stoppeddropping() -- ensure stopped is called
  love.window.focus()
  require("util.sceneManager").changeScene("scene.editor", project, start)
end
